...
%>
		...
		public void Hydrate<%=pkMethod%>Collection(<%=pkClassQualified%>Collection collToFilter)
        {
            if (col<%=pkMethod%> != null)
                col<%=pkMethod%>.Clear(); 
            else                
                col<%=pkMethod%> = new <%=pkClassQualified%>Collection();
                
            col<%=pkMethod%>.CopyFrom(collToFilter.Where(<%=pkTbl.ClassName%>.Columns.<%=pkColumn%>, Comparison.Equals, this.<%=tbl.PrimaryKey.PropertyName%>).Filter());
            
            col<%=pkMethod%>.ListChanged += new ListChangedEventHandler(col<%=pkMethod%>_ListChanged);
        }
		public void Hydrate<%=pkMethod%>Collection(DataTable dtToFilter)
        {
            if (col<%=pkMethod%> != null)
                col<%=pkMethod%>.Clear(); 
            else                
            ...
            //todo ensure schema here
            foreach (DataRow dr in dtToFilter.Rows)
            {   
                if(dr.ItemArray[dr.Table.Columns.IndexOf("<%=pkColumn%>")] != null && 
                    dr.ItemArray[dr.Table.Columns.IndexOf("<%=pkColumn%>")].ToString().Trim().Length > 0 && 
                    (<%=tbl.PrimaryKey.DataType%>)dr.ItemArray.GetValue(dr.Table.Columns.IndexOf("<%=pkColumn%>")) == this.<%=tbl.PrimaryKey.PropertyName%>)
                {
                    <%=pkClassQualified%> item = new <%=pkClassQualified%>();
                    item.Load(dr);
                    col<%=pkMethod%>.Add(item);                    
                }
            }
...
%>
		...
		public <%=fkClassQualified%> <%=fkMethod%>Record
		{
            ...
            set
            {
                ...
                this.<%=string.Format("{0}{1}", fkColumnID.Substring(0, 1).ToUpper(), fkColumnID.Substring(1, fkColumnID.Length - 1))%> = value.<%=fkTbl.PrimaryKey.PropertyName%>;
            }
		}
		<%
            if (fkTbl.PrimaryKey.DataType.ToString().ToLower() == "int32")
        {%>
		public void Hydrate<%=fkMethod%>Record(<%=fkClassQualified%>Collection <%=fkClass%>CollToFilter)
		{
		    if (_<%=fkMethod.ToLower()%>record != null)
		        _<%=fkMethod.ToLower()%>record = null;
		        
		    if(this.<%=string.Format("{0}{1}", fkColumnID.Substring(0, 1).ToUpper(), fkColumnID.Substring(1, fkColumnID.Length - 1))%> > 0)
		    {
		        _<%=fkMethod.ToLower()%>record = new <%=fkClassQualified%>();
		        _<%=fkMethod.ToLower()%>record.CopyFrom((<%=fkClassQualified%>)<%=fkClass%>CollToFilter.Find(this.<%=string.Format("{0}{1}", fkColumnID.Substring(0, 1).ToUpper(), fkColumnID.Substring(1, fkColumnID.Length - 1))%>));
		    }
		}
		<%} %>
		public void Hydrate<%=fkMethod%>Record(DataTable dtToFilter)
		{
		    if (_<%=fkMethod.ToLower()%>record != null)
		        _<%=fkMethod.ToLower()%>record = null;
            ...
            foreach (DataRow dr in dtToFilter.Rows)
            {
                if(dr.ItemArray[dr.Table.Columns.IndexOf("<%=fkTbl.PrimaryKey.PropertyName%>")] != null && 
                    dr.ItemArray[dr.Table.Columns.IndexOf("<%=fkTbl.PrimaryKey.PropertyName%>")].ToString().Trim().Length > 0 &&
                    (<%=fkTbl.PrimaryKey.DataType%>)dr.ItemArray.GetValue(dr.Table.Columns.IndexOf("<%=fkTbl.PrimaryKey.PropertyName%>")) == this.<%=string.Format("{0}{1}", fkColumnID.Substring(0, 1).ToUpper(), fkColumnID.Substring(1, fkColumnID.Length - 1))%>)
                {
                    _<%=fkMethod.ToLower()%>record = new <%=fkClassQualified%>();
                    _<%=fkMethod.ToLower()%>record.Load(dr);
                    break;
                }
            }
		}	
