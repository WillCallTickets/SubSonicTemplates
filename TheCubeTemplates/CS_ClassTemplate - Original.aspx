...
%>
		...
		public void Hydrate<%=pkMethod%>Collection(DataTable dt)
        {
            ...
            //can we match and ensure schema here?
...
%>
		...
		public <%=fkClassQualified%> <%=fkMethod%>
		{
			get 
			{
			    if(_<%=fkMethod.ToLower()%>record == null) 
			        _<%=fkMethod.ToLower()%>record = <%=fkClassQualified%>.FetchByID(this.<%=fkID%>); 
		...
		//we use a datatable so that we can travers for other objects
		public void Hydrate<%=fkMethod%>Record(DataTable dt)
		{
		    if (_<%=fkMethod.ToLower()%>record != null)
		        _<%=fkMethod.ToLower()%>record = null;
